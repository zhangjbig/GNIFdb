...
library(stringr)
aaproperties <- which(str_length(des)==1)
fasgai <- grep("^fasgaiVectors",colnames(fea))
zScale <- grep("^zScales",colnames(fea))
tScale<- grep("^tScale",colnames(fea))
stScale<- grep("^stScale",colnames(fea))
vhseScale<- grep("^vhseScale",colnames(fea))
cruciani <- grep("^cruciani",colnames(fea))
mswhimScore <- grep("^mswhimScore",colnames(fea))
kideraFactor <- grep("^kideraFactor",colnames(fea))
blosumIndice <- grep("^blosumIndice",colnames(fea))
protFP <- grep("^protFP",colnames(fea))

classify <- grep("^Tiny|^Small|^Aliphatic|^Aromatic|^Nonpolar|^Polar|^Charged|^Basic|^Acidic",colnames(fea))


fasgai_result <- cbind(result[,1:3],result[,fasgai])
phyche <- cbind(result[,1:3],result[,-unique(c(fasgai,zScale,tScale,stScale,vhseScale,mswhimScore
                                               ,kideraFactor,blosumIndice,protFP,cruciani,aaproperties))])
phyche <- phyche[,-c(14,15,16)]
cla_result <-  cbind(result[,1:3],result[,classify])
aaproperties_result <- cbind(result[,1:3],result[,aaproperties])
blosum_result <- cbind(result[,1:3],result[,blosumIndice])
zScale_result <-  cbind(result[,1:3],result[,zScale])
tScale_result<-  cbind(result[,1:3],result[,tScale])
stScale_result<-  cbind(result[,1:3],result[,stScale])
vhseScale_result<-  cbind(result[,1:3],result[,vhseScale])
cruciani_result<-  cbind(result[,1:3],result[,cruciani])
mswhimScore_result<-  cbind(result[,1:3],result[,mswhimScore])
kideraFactor_result<-  cbind(result[,1:3],result[,kideraFactor])
blosumIndice_result<-  cbind(result[,1:3],result[,blosumIndice])
protFP_result<-  cbind(result[,1:3],result[,protFP])


cat("计算完毕")

write.csv(protFP_result,file =  "protfp_result.csv")
cat("写入完毕")
